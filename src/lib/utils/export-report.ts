...
// ============================================
// Fetch company info (logo + name) automatically
// ============================================
...
export async function exportToExcel(config: ExportConfig) {
  try {
    ...
    const footerRow = sheet.addRow([
      config.lang === "ar"
        ? `ØªÙ… Ø§Ù„Ø¥Ù†Ø´Ø§Ø¡ Ø¨ÙˆØ§Ø³Ø·Ø© MotionHR â€” ${new Date().toLocaleDateString("ar-EG")}`
        : `Generated by MotionHR â€” ${new Date().toLocaleDateString("en-US")}`
    ]);
    ...
    toast.success(config.lang === "ar" ? "ØªÙ… ØªØ­Ù…ÙŠÙ„ Ø§Ù„Ù…Ù„Ù Ø¨Ù†Ø¬Ø§Ø­" : "File downloaded successfully");
  } catch (error) {
    console.error("Excel export error:", error);
    toast.error(config.lang === "ar" ? "ÙØ´Ù„ ØªØ­Ù…ÙŠÙ„ Ø§Ù„Ù…Ù„Ù" : "Failed to download file");
  }
}

// ============================================
// PDF EXPORT â€” Ø§Ø­ØªØ±Ø§ÙÙŠ Ù…Ø¹ Ø¯Ø¹Ù… Ø§Ù„Ø¹Ø±Ø¨ÙŠ
// ============================================
export async function exportToPDF(config: ExportConfig) {
  try {
    ...
    if (config.subtitle) {
      ...
      doc.setFont("Cairo", "normal");
      doc.text(config.subtitle, pageWidth / 2, 46, { align: "center" });
    }
    ...
    if (config.period) {
      ...
      doc.setTextColor(107, 114, 128);
      doc.setFont("Cairo", "normal");
      doc.text(config.period, pageWidth / 2, 52, { align: "center" });
    }
    ...
    autoTable(doc, {
      ...
      body: config.data.map(row =>
        config.columns.map(col => {
          const val = row[col.key];
          if (col.formatter) return col.formatter(val, row);
          return val !== undefined && val !== null ? String(val) : "â€”";
        })
      ),
      theme: "striped",
      styles: {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        isRTL ? `ØµÙØ­Ø© ${i} Ù…Ù† ${pageCount}` : `Page ${i} of ${pageCount}`,
        pageWidth - 14,
        pageHeight - 8,
        { align: "right" }
      );
