...
import { ArrowLeft, ArrowRight, Calendar, Plus, Loader2 } from "lucide-react";
...
export default function PoliciesPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/hr/policies-leave", { headers: { Authorization: authHeader } })
      .then(r => r.json())
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">{d.leavePoliciesTitle}</h1>
            <p className="text-muted-foreground mt-1 text-sm">{d.leavePoliciesDesc}</p>
          </div>
        </div>
