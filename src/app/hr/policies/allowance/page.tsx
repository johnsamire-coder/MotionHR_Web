...
import { ArrowLeft, ArrowRight, TrendingUp, Plus, Loader2 } from "lucide-react";
...
export default function PoliciesPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/hr/policies-allowance", { headers: { Authorization: authHeader } })
      .then(r => r.json())
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">{d.allowancePoliciesTitle}</h1>
            <p className="text-muted-foreground mt-1 text-sm">{d.allowancePoliciesDesc}</p>
          </div>
        </div>
